...
can also use Gradle. It also shows values of `com.example` and `rest-service-cors` as the
...
If you would like to include all paths instead of a specific path, you can replace `/greeting-javaconfig` with the pattern `/**`.
And in order to allow any origin, you can leave out the `allowedOrigins` method call.
