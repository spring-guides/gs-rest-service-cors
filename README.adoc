...
:images: https://github.com/spring-guides/gs-rest-service-cors/raw/master/images
:toc:
:icons: font
...
You will build a service that accepts HTTP POST requests at
...
`src/main/java/com/example/restservicecors/GreetingController.java`) handles `POST`
...
NOTE: The preceding example uses the `@PostMapping` annotation, which acts as a shortcut
for `@RequestMapping(method = RequestMethod.POST)`. We use `POST` in this case because it
requires the browser to send a CORS preflight request (see below).
...
Now that the service is up, use `curl` (or your favourite HTTP client) to `POST` to `http://localhost:8080/greeting`

====
curl localhost:8080/greeting -X POST
====
